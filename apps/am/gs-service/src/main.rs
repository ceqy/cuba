...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    // Bootstrap Service
    let context = cuba_service::ServiceBootstrapper::run(50086).await?;
    ...
    let repo = Arc::new(SettingsRepository::new(pool.clone()));
    let handler = Arc::new(SettingsHandler::new(repo.clone()));
    ...
    Server::builder()
        .add_service(GeoServiceServer::new(service))
