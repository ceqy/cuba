...
use so_service::application::handlers::{
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // API
    let so_service = SoServiceImpl::new(create_handler, get_handler, list_handler);

    // Reflection Service
    let reflection_service = tonic_reflection::server::Builder::configure()
        .register_encoded_file_descriptor_set(
            so_service::api::proto::sd::so::v1::FILE_DESCRIPTOR_SET,
        )
        .build_v1()?;
