use crate::application::commands::{
    CreateSalesOrderCommand, CreateSalesOrderItemCommand, GetSalesOrderQuery, ListSalesOrdersQuery,
};
use crate::application::handlers::{
    CreateSalesOrderHandler, GetSalesOrderHandler, ListSalesOrdersHandler,
};
use std::sync::Arc;
...
impl SalesOrderFulfillmentService for SoServiceImpl {
    async fn create_sales_order(
        ...
        let cmd = CreateSalesOrderCommand {
            ...
            items: req
                .items
                .into_iter()
                .map(|i| CreateSalesOrderItemCommand {
        ...
        if let Some(o) = order {
            Ok(Response::new(SalesOrderDetail {
                ...
                header: Some(SalesOrderHeader {
                    ...
                    document_date: Some(to_timestamp(
                        o.document_date.and_hms_opt(0, 0, 0).unwrap().and_utc(),
                    )),
                    // ... map other fields
                    ..Default::default()
                }),
    ...
    async fn list_sales_orders(
        ...
        let orders = self
            .list_handler
            .handle(ListSalesOrdersQuery {
                sold_to_party: if req.sold_to_party.is_empty() {
                    None
                } else {
                    Some(req.sold_to_party)
                },
                limit: 20,
            })
            .await
    ...
    async fn update_sales_order(
        &self,
        _r: Request<UpdateSalesOrderRequest>,
    ) -> Result<Response<SalesOrderResponse>, Status> {
        Err(Status::unimplemented(""))
    }
    async fn cancel_sales_order(
        &self,
        _r: Request<CancelSalesOrderRequest>,
    ) -> Result<Response<SalesOrderResponse>, Status> {
        Err(Status::unimplemented(""))
    }
    async fn get_sales_order_fulfillment_status(
        &self,
        _r: Request<GetSalesOrderRequest>,
    ) -> Result<Response<FulfillmentStatusResponse>, Status> {
        Err(Status::unimplemented(""))
    }
}
