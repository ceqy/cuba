...
impl SalesAnalyticsService for AnServiceImpl {
    async fn get_sales_by_customer(
        ...
        let results = self
            .handler
            .analyze_by_customer(cmd)
            .await
            .map_err(|e| Status::internal(e.to_string()))?;
        Ok(Response::new(SalesAnalysisResponse {
            results: results
                .into_iter()
                .map(|r| SalesDimension {
                    id: r.id,
                    name: r.name,
                    revenue: Some(common_v1::MonetaryValue {
                        value: r.revenue.to_string(),
    ...
    async fn export_sales_report(
        &self,
        _r: Request<ExportSalesReportRequest>,
