...
use std::sync::Arc;
use crate::application::commands::{CalculatePriceCommand, UpdateConditionCommand, PricingItemInput};
...
impl PricingEngineService for PeServiceImpl {
    ...
    async fn calculate_price(
        ...
        let cmd = CalculatePriceCommand {
            ...
            customer: if ctx.customer.is_empty() { None } else { Some(ctx.customer) },
            pricing_date: chrono::Utc::now().date_naive(),
            items: req.items.into_iter().map(|i| PricingItemInput {
                item_id: i.item_id,
                material: i.material,
                quantity: Decimal::from_str(&i.quantity).unwrap_or_else(|_| Decimal::new(1, 0)),
            }).collect(),
        };
        let results = self.handler.calculate_price(cmd).await
            .map_err(|e| Status::internal(e.to_string()))?;
        Ok(Response::new(CalculatePriceResponse {
            results: results.into_iter().map(|r| PricingResult {
                item_id: r.item_id,
                net_price: Some(common_v1::MonetaryValue { value: r.net_price.to_string(), currency_code: "CNY".to_string() }),
                tax_amount: Some(common_v1::MonetaryValue { value: r.tax_amount.to_string(), currency_code: "CNY".to_string() }),
                gross_price: Some(common_v1::MonetaryValue { value: r.gross_price.to_string(), currency_code: "CNY".to_string() }),
                conditions: r.conditions.into_iter().map(|c| PricingCondition {
    ...
    async fn update_pricing_condition(
        ...
        let cmd = UpdateConditionCommand {
            ...
            material: if req.material.is_empty() { None } else { Some(req.material) },
            customer: if req.customer.is_empty() { None } else { Some(req.customer) },
        ...
        let id = self.handler.update_condition(cmd).await
            .map_err(|e| Status::internal(e.to_string()))?;
