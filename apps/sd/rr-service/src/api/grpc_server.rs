...
impl RevenueRecognitionService for RrServiceImpl {
    ...
    async fn create_revenue_contract(
        ...
        let contract_num = self.handler.create_contract(cmd).await
    ...
    async fn get_performance_obligations(
        ...
        let req = request.into_inner();
        let c = self.repo.find_contract_by_number(&req.revenue_contract_number).await
            .map_err(|e| Status::internal(e.to_string()))?
            .ok_or_else(|| Status::not_found("Contract not found"))?;
        Ok(Response::new(GetPerformanceObligationsResponse {
            obligations: c.obligations.into_iter().map(|pob| PerformanceObligation {
                pob_id: pob.pob_code,
                ...
                allocated_price: pob.allocated_price.map(|a| common_v1::MonetaryValue { value: a.to_string(), currency_code: pob.currency.clone() }),
                recognition_method: pob.recognition_method,
                recognized_revenue: Some(common_v1::MonetaryValue { value: pob.recognized_revenue.to_string(), currency_code: pob.currency.clone() }),
                deferred_revenue: Some(common_v1::MonetaryValue { value: pob.deferred_revenue.to_string(), currency_code: pob.currency }),
    ...
    async fn run_revenue_posting(
        ...
        let (run_id, doc_count) = self.handler.run_posting(cmd).await
            .map_err(|e| Status::internal(e.to_string()))?;
    ...
    async fn reverse_revenue_posting(&self, _r: Request<ReverseRevenuePostingRequest>) -> Result<Response<RevenuePostingResponse>, Status> { Err(Status::unimplemented("")) }
    async fn get_revenue_postings(&self, _r: Request<GetRevenuePostingsRequest>) -> Result<Response<GetRevenuePostingsResponse>, Status> { Err(Status::unimplemented("")) }
}
