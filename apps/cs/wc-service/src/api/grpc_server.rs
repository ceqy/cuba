...
impl WarrantyClaimsService for WcServiceImpl {
    async fn create_claim(
        ...
        let cmd = CreateClaimCommand {
            ...
            failure_description: if claim.failure_description.is_empty() {
                None
            } else {
                Some(claim.failure_description)
            },
    ...
    async fn get_claim(
        ...
        Ok(Response::new(ClaimDetail {
            ...
            claimed_amount: Some(common_v1::MonetaryValue {
                value: c.claimed_amount.to_string(),
            ...
            adjudication: c.adjudication.map(|a| AdjudicationResult {
                ...
                approved_amount: a.approved_amount.map(|v| common_v1::MonetaryValue {
                    value: v.to_string(),
    ...
    async fn adjudicate_claim(
        ...
        let cmd = AdjudicateClaimCommand {
            ...
            approved_amount: result
                .approved_amount
                .map(|a| Decimal::from_str(&a.value).unwrap_or_default()),
            notes: if result.notes.is_empty() {
                None
            } else {
                Some(result.notes)
            },
    ...
    async fn update_claim(
        &self,
        _r: Request<UpdateClaimRequest>,
    ) -> Result<Response<ClaimResponse>, Status> {
        Err(Status::unimplemented(""))
    }
    async fn list_claims(
        &self,
        _r: Request<ListClaimsRequest>,
    ) -> Result<Response<ListClaimsResponse>, Status> {
        Err(Status::unimplemented(""))
    }
    async fn add_evidence_attachment(
        &self,
        _r: Request<AddEvidenceAttachmentRequest>,
    ) -> Result<Response<ClaimResponse>, Status> {
        Err(Status::unimplemented(""))
    }
    async fn close_claim(
        &self,
        _r: Request<CloseClaimRequest>,
    ) -> Result<Response<ClaimResponse>, Status> {
        Err(Status::unimplemented(""))
    }
}
