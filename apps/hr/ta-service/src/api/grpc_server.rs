use crate::application::commands::{
    CreateJobApplicationCommand, ScheduleInterviewCommand, UpdateStatusCommand,
};
...
impl TalentAcquisitionService for TaServiceImpl {
    async fn create_job_application(
        ...
        let cmd = CreateJobApplicationCommand {
            ...
            phone: if candidate.phone.is_empty() {
                None
            } else {
                Some(candidate.phone)
            },
            resume_url: if candidate.resume_url.is_empty() {
                None
            } else {
                Some(candidate.resume_url)
            },
        };
        let app_id = self
            .handler
            .create_application(cmd)
            .await
    ...
    async fn schedule_interview(
        ...
        let cmd = ScheduleInterviewCommand {
            ...
            interviewer_id: if interview.interviewer_id.is_empty() {
                None
            } else {
                Some(interview.interviewer_id)
            },
            location: if interview.location.is_empty() {
                None
            } else {
                Some(interview.location)
            },
        };
        let interview_id = self
            .handler
            .schedule_interview(cmd)
            .await
            .map_err(|e| Status::internal(e.to_string()))?;
    ...
    async fn submit_resume(
        &self,
        _r: Request<SubmitResumeRequest>,
    ) -> Result<Response<JobApplicationResponse>, Status> {
        Err(Status::unimplemented(""))
    }
    async fn list_job_applications(
        &self,
        _r: Request<ListJobApplicationsRequest>,
    ) -> Result<Response<ListJobApplicationsResponse>, Status> {
        Err(Status::unimplemented(""))
    }
}
