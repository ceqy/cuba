...
use crate::infrastructure::services::{JwtService, CryptoService, ClientSecretService};
...
impl<C: ClientRepository, A: AuthCodeRepository> AuthorizeHandler<C, A> {
    pub fn new(client_repo: Arc<C>, auth_code_repo: Arc<A>, crypto_service: Arc<CryptoService>) -> Self {
        Self { client_repo, auth_code_repo, crypto_service }
    }
...
impl<C: ClientRepository, A: AuthCodeRepository, R: RefreshTokenRepository> TokenHandler<C, A, R> {
    pub fn new(
        ...
        Self { client_repo, auth_code_repo, refresh_token_repo, jwt_service, crypto_service, secret_service }
    }
    ...
    pub async fn handle_auth_code(
        ...
        let auth_code = self.auth_code_repo.find_by_code(&code).await?
        ...
        if !self.secret_service.verify_secret(&client_secret, &client.client_secret)? {
             return Err(anyhow::anyhow!("Invalid client secret"));
        }

        // PKCE Verification if applicable
        if let Some(challenge) = auth_code.code_challenge {
            let verifier = code_verifier.ok_or_else(|| anyhow::anyhow!("Code verifier required for PKCE"))?;
            let method = auth_code.code_challenge_method.unwrap_or_else(|| "plain".to_string());
            if !self.crypto_service.verify_pkce(&verifier, &challenge, &method) {
        ...
        // Generate Tokens
        let access_token = self.jwt_service.generate_access_token(&auth_code.user_id, &client_id, "default", &auth_code.scope)?;
    ...
    pub async fn handle_client_credentials(
        ...
        let access_token = self.jwt_service.generate_access_token(&format!("client:{}", client_id), &client_id, "default", &scope)?;
    ...
    pub async fn handle_refresh_token(
        ...
        let rt = self.refresh_token_repo.find_by_token(&token).await?
        ...
        let access_token = self.jwt_service.generate_access_token(&rt.user_id, &client_id, "default", &rt.scope)?;
