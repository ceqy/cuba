    ...
    R: RefreshTokenRepository + 'static 
{
    pub fn new(
        ...
        Self { authorize_handler, token_handler, client_repo, refresh_token_repo, secret_service, jwt_service }
    }
}
    ...
    R: RefreshTokenRepository + 'static 
{
    async fn authorize(&self, request: Request<AuthorizeRequest>) -> Result<Response<AuthorizeResponse>, Status> {
        ...
        let code = self.authorize_handler.handle(
            req.client_id,
            user_id,
            req.redirect_uri,
            req.scope,
            if req.code_challenge.is_empty() { None } else { Some(req.code_challenge) },
    ...
    async fn token(&self, request: Request<TokenRequest>) -> Result<Response<TokenResponse>, Status> {
        let req = request.into_inner();
        
        match req.grant_type.as_str() {
            "authorization_code" => {
                let (access_token, refresh_token, expires_in) = self.token_handler.handle_auth_code(
                    req.code,
                    req.client_id,
                    req.client_secret,
                    req.redirect_uri,
                    if req.code_verifier.is_empty() { None } else { Some(req.code_verifier) },
            ...
            "client_credentials" => {
                let (access_token, expires_in) = self.token_handler.handle_client_credentials(
                    req.client_id,
                    req.client_secret,
            ...
            "refresh_token" => {
                let (access_token, refresh_token, expires_in) = self.token_handler.handle_refresh_token(
                    req.refresh_token,
                    req.client_id,
                    req.client_secret,
                ).await.map_err(|e| Status::unauthenticated(e.to_string()))?;
    ...
    async fn revoke_token(&self, request: Request<RevokeTokenRequest>) -> Result<Response<RevokeTokenResponse>, Status> {
        let req = request.into_inner();
        ...
        match req.token_type_hint.as_str() {
            "refresh_token" | "" => {
                ...
                self.refresh_token_repo.delete(&req.token).await
    ...
    async fn user_info(&self, request: Request<UserInfoRequest>) -> Result<Response<UserInfoResponse>, Status> {
        let req = request.into_inner();
        
        // Decode the access token
        let claims = self.jwt_service.decode_token(&req.access_token)
    ...
    async fn create_client(&self, request: Request<CreateClientRequest>) -> Result<Response<OAuthClient>, Status> {
        let req = request.into_inner();
        
        // Generate client secret
        let raw_secret = uuid::Uuid::new_v4().to_string().replace("-", "");
        let hashed_secret = self.secret_service.hash_secret(&raw_secret).map_err(|e| Status::internal(e.to_string()))?;
        ...
        self.client_repo.save(&client).await.map_err(|e| Status::internal(e.to_string()))?;
        ...
        Ok(Response::new(OAuthClient {
            ...
            created_at: Some(prost_types::Timestamp::from(std::time::SystemTime::from(client.created_at))),
        }))
    }

    async fn list_clients(&self, _request: Request<ListClientsRequest>) -> Result<Response<ListClientsResponse>, Status> {
        let clients = self.client_repo.list_all().await.map_err(|e| Status::internal(e.to_string()))?;
        
        Ok(Response::new(ListClientsResponse {
            clients: clients.into_iter().map(|c| OAuthClient {
                ...
                scopes: c.scopes,
                created_at: Some(prost_types::Timestamp::from(std::time::SystemTime::from(c.created_at))),
            }).collect(),
            pagination: None,
        }))
    }

    async fn delete_client(&self, request: Request<DeleteClientRequest>) -> Result<Response<DeleteClientResponse>, Status> {
        let req = request.into_inner();
        self.client_repo.delete(&req.client_id).await.map_err(|e| Status::internal(e.to_string()))?;
