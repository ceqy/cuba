...
#[tonic::async_trait]
impl<R: RoleRepository + 'static, P: PermissionRepository + 'static> RbacService for RBACServiceImpl<R, P> {
    async fn create_role(&self, request: Request<CreateRoleRequest>) -> Result<Response<common_proto::Role>, Status> {
        let req = request.into_inner();
        let role = crate::domain::Role {
            id: uuid::Uuid::new_v4().to_string(),
            name: req.name,
            description: req.description,
            parent_id: if req.parent_id.is_empty() { None } else { Some(req.parent_id) },
            tenant_id: req.tenant_id,
            is_immutable: false,
            created_at: chrono::Utc::now(),
        };
        self.role_repository.save(&role).await.map_err(|e| Status::internal(e.to_string()))?;
        Ok(Response::new(common_proto::Role {
            ...
            created_at: Some(prost_types::Timestamp::from(std::time::SystemTime::from(role.created_at))),
        }))
    }
    ...
    async fn list_roles(&self, _request: Request<ListRolesRequest>) -> Result<Response<ListRolesResponse>, Status> {
        let roles = self.role_repository.find_all().await.map_err(|e| Status::internal(e.to_string()))?;
    ...
    async fn list_permissions(&self, _request: Request<common_proto::PaginationRequest>) -> Result<Response<ListPermissionsResponse>, Status> {
        let perms = self.permission_repository.find_all().await.map_err(|e| Status::internal(e.to_string()))?;
        Ok(Response::new(ListPermissionsResponse {
            permissions: perms.into_iter().map(|p| common_proto::Permission {
                permission_id: p.id,
                code: p.code,
                resource: p.resource,
                action: p.action,
                description: p.description,
                created_at: Some(prost_types::Timestamp::from(std::time::SystemTime::from(p.created_at))),
                conditions: None,
                effect: 0,
            }).collect(),
            pagination: None,
    ...
    async fn grant_permissions_to_role(&self, request: Request<GrantPermissionsRequest>) -> Result<Response<GrantPermissionsResponse>, Status> {
        let req = request.into_inner();
        self.role_repository.grant_permissions(&req.role_id, &req.permission_ids).await.map_err(|e| Status::internal(e.to_string()))?;
    ...
    async fn assign_role_to_user(&self, request: Request<AssignRoleToUserRequest>) -> Result<Response<AssignRoleToUserResponse>, Status> {
        let req = request.into_inner();
        self.role_repository.assign_to_user(&req.user_id, &req.role_id).await.map_err(|e| Status::internal(e.to_string()))?;
    ...
    async fn remove_role_from_user(&self, request: Request<RemoveRoleFromUserRequest>) -> Result<Response<RemoveRoleFromUserResponse>, Status> {
        let req = request.into_inner();
        self.role_repository.remove_from_user(&req.user_id, &req.role_id).await.map_err(|e| Status::internal(e.to_string()))?;
    ...
    async fn check_permissions(&self, request: Request<CheckPermissionsRequest>) -> Result<Response<CheckPermissionsResponse>, Status> {
        let req = request.into_inner();
        let perms = self.permission_repository.find_by_user_id(&req.user_id).await.map_err(|e| Status::internal(e.to_string()))?;
