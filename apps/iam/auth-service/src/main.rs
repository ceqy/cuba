...
use auth_service::application::handlers::{
    LoginUserHandler, RefreshTokenHandler, RegisterUserHandler,
};
...
use auth_service::infrastructure::persistence::{
    PostgresUserRepository, PostgresUserSessionRepository,
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let rbac_addr = std::env::var("RBAC_SERVICE_ADDR")
        .unwrap_or_else(|_| "http://rbac-service.cuba-iam.svc.cluster.local:50052".to_string());
    let rbac_client = Arc::new(RbacClient::new(rbac_addr).await?);

    // Handlers
    let register_handler = Arc::new(RegisterUserHandler::new(
        user_repo.clone(),
