...
-- 复合索引（业务分析）- using transaction_type and reference_transaction_type only
CREATE INDEX IF NOT EXISTS idx_journal_lines_business_analysis
ON journal_entry_lines(transaction_type, reference_transaction_type);
...
SELECT
  ...
  COUNT(DISTINCT je.document_number) as document_count,
...
WHERE jel.trading_partner_company IS NOT NULL
  AND jel.trading_partner_company != je.company_code
GROUP BY
  je.company_code,
  jel.trading_partner_company,
  je.fiscal_year,
...
BEGIN
  -- Get values from parent journal_entries table
  SELECT company_code, fiscal_year, document_date
  INTO v_company_code, v_fiscal_year, v_document_date
