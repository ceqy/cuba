...
SELECT
    ...
    CASE
        ...
        THEN jel.local_amount - (jel.amount * CAST(jel.exchange_rate AS DECIMAL(10,6)))
        ELSE 0
...
SELECT
    ...
    SUM(jel.local_amount) as total_local_currency_amount,
    -- 计算理论本位币金额
    SUM(jel.amount * COALESCE(jel.exchange_rate, 1.0)) as calculated_local_amount,
    -- 计算汇兑差异
    SUM(jel.local_amount) - SUM(jel.amount * COALESCE(jel.exchange_rate, 1.0)) as total_exchange_variance,
    -- 计算平均汇率
    AVG(COALESCE(jel.exchange_rate, 1.0)) as average_exchange_rate
FROM journal_entries je
JOIN journal_entry_lines jel ON je.id = jel.journal_entry_id
WHERE je.status = 'POSTED'
...
GROUP BY
    ...
    je.local_currency,
    jel.exchange_rate
HAVING SUM(jel.local_amount) - SUM(jel.amount * COALESCE(jel.exchange_rate, 1.0)) != 0
ORDER BY
    je.company_code,
    je.fiscal_year,
    je.fiscal_period;
...
WHERE je.status = 'POSTED'
  AND je.currency != je.local_currency
  AND (jel.exchange_rate IS NULL OR CAST(jel.exchange_rate AS TEXT) = '' OR jel.exchange_rate = 0)
...
WHERE je.status = 'POSTED'
  ...
  AND jel.exchange_rate != 0
  AND jel.exchange_rate != 1.0
  AND jel.exchange_rate != 1
