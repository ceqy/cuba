...
use std::sync::Arc;
use crate::application::commands::{CreateContractCommand, ApproveContractCommand, ContractItemInput};
...
impl ContractManagementService for CtServiceImpl {
    ...
    async fn create_contract(
        ...
        let cmd = CreateContractCommand {
            ...
            validity_start: None,
            validity_end: None,
            target_value: header.target_value.map(|v| Decimal::from_str(&v.value).unwrap_or_default()),
            items: req.items.into_iter().map(|i| ContractItemInput {
                item_number: i.item_number,
                material: if i.material.is_empty() { None } else { Some(i.material) },
                short_text: if i.short_text.is_empty() { None } else { Some(i.short_text) },
                target_quantity: i.target_quantity.map(|q| Decimal::from_str(&q.value).unwrap_or_default()),
                net_price: i.net_price.map(|p| Decimal::from_str(&p.value).unwrap_or_default()),
                plant: if i.plant.is_empty() { None } else { Some(i.plant) },
            }).collect(),
        };
        let num = self.handler.create_contract(cmd).await
    ...
    async fn get_contract(
        ...
        let req = request.into_inner();
        let c = self.repo.find_by_number(&req.contract_number).await
            .map_err(|e| Status::internal(e.to_string()))?
        ...
        Ok(Response::new(ContractDetail {
            ...
            header: Some(ContractHeader {
                ...
                validity_start: None,
                validity_end: None,
                target_value: c.target_value.map(|v| common_v1::MonetaryValue { value: v.to_string(), currency_code: c.currency.clone() }),
                release_status: common_v1::ReleaseStatus::NotReleased as i32,
            }),
            items: c.items.into_iter().map(|i| ContractItem {
                item_number: i.item_number,
                material: i.material.unwrap_or_default(),
                short_text: i.short_text.unwrap_or_default(),
                target_quantity: i.target_quantity.map(|q| common_v1::QuantityValue { value: q.to_string(), unit_code: i.unit.clone() }),
                unit: i.unit,
                net_price: i.net_price.map(|p| common_v1::MonetaryValue { value: p.to_string(), currency_code: i.price_currency }),
                price_unit: 1,
    ...
    async fn approve_contract(
        ...
        self.handler.approve_contract(cmd).await
            .map_err(|e| Status::internal(e.to_string()))?;
        Ok(Response::new(ContractResponse {
            success: true,
    ...
    async fn update_contract(&self, _r: Request<UpdateContractRequest>) -> Result<Response<ContractResponse>, Status> { Err(Status::unimplemented("")) }
    async fn list_contracts(&self, _r: Request<ListContractsRequest>) -> Result<Response<ListContractsResponse>, Status> { Err(Status::unimplemented("")) }
}
