...
impl SpendAnalyticsService for SaServiceImpl {
    ...
    async fn get_spend_by_category(
        ...
        let results = self.handler.analyze_by_category(cmd).await
            .map_err(|e| Status::internal(e.to_string()))?;
        Ok(Response::new(SpendAnalysisResponse {
            results: results.into_iter().map(|r| SpendDimension {
                id: r.id,
                name: r.name,
                spend_amount: Some(common_v1::MonetaryValue { value: r.spend_amount.to_string(), currency_code: r.currency }),
    ...
    async fn get_spend_by_supplier(
        ...
        let results = self.handler.analyze_by_supplier(cmd).await
            .map_err(|e| Status::internal(e.to_string()))?;
        Ok(Response::new(SpendAnalysisResponse {
            results: results.into_iter().map(|r| SpendDimension {
                id: r.id,
    ...
    async fn get_spend_trend(
        ...
        Ok(Response::new(SpendTrendResponse {
            trend: trend.into_iter().map(|t| TimeSeriesDataPoint {
                period: t.period,
                spend_amount: Some(common_v1::MonetaryValue { value: t.spend_amount.to_string(), currency_code: t.currency }),
            }).collect(),
        }))
    }
}
