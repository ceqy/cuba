...
impl SourcingEventService for SeServiceImpl {
    async fn create_rfq(
        ...
        let rfq_detail = req
            .rfq
            .ok_or_else(|| Status::invalid_argument("RFQ detail required"))?;
        let cmd = CreateRFQCommand {
            company_code: rfq_detail.company_code,
            purchasing_org: if rfq_detail.purchasing_org.is_empty() {
                None
            } else {
                Some(rfq_detail.purchasing_org)
            },
        };
        let rfq_num = self
            .handler
    ...
    async fn get_rfq(
        ...
        let rfq = self
            .repo
            .find_rfq_by_number(&req.rfq_number)
            .await
            .map_err(|e| Status::internal(e.to_string()))?
        ...
        Ok(Response::new(RfqDetail {
            ...
            quote_deadline: rfq.quote_deadline.map(|d| prost_types::Timestamp {
            ...
            items: rfq
                ...
                .map(|item| RfqItem {
                    ...
                    quantity: item.quantity.map(|q| common_v1::QuantityValue {
                        value: q.to_string(),
                        unit_code: item.unit.clone(),
                    }),
                    delivery_date: item.delivery_date.map(|d| prost_types::Timestamp {
                        seconds: d.and_hms_opt(0, 0, 0).unwrap().and_utc().timestamp(),
                        nanos: 0,
                    }),
    ...
    async fn submit_supplier_quote(
        ...
        let req = request.into_inner();
        let quote_detail = req
            .quote
            .ok_or_else(|| Status::invalid_argument("Quote detail required"))?;
    ...
    async fn award_quote(
        ...
        let req = request.into_inner();
        let cmd = AwardCommand {
            quote_number: req.quote_number.clone(),
        };
        let _success = self
            .handler
            .award_quote(cmd)
    ...
    async fn publish_rfq(
        &self,
        _r: Request<PublishRfqRequest>,
    ) -> Result<Response<RfqResponse>, Status> {
        Err(Status::unimplemented(""))
    }
    async fn close_rfq(
        &self,
        _r: Request<CloseRfqRequest>,
    ) -> Result<Response<RfqResponse>, Status> {
        Err(Status::unimplemented(""))
    }
    async fn get_supplier_quote(
        &self,
        _r: Request<GetQuoteRequest>,
    ) -> Result<Response<QuoteDetail>, Status> {
        Err(Status::unimplemented(""))
    }
}
