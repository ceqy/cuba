...
impl PurchaseOrderService for PoServiceImpl {
        ...
        let cmd = CreatePurchaseOrderCommand {
            ...
            items: req.items.into_iter().map(|i| {
                ...
                CreatePurchaseOrderItemCommand {
                    ...
                    net_price: i.net_price.unwrap_or_default().value.parse().unwrap_or_default(),
                }
            }).collect(),
        };

        let number = self.create_handler.handle(cmd).await
    ...
    // Stubs
    async fn update_purchase_order(&self, _r: Request<UpdatePurchaseOrderRequest>) -> Result<Response<PurchaseOrderResponse>, Status> { Err(Status::unimplemented("")) }
    async fn cancel_purchase_order(&self, _r: Request<CancelPurchaseOrderRequest>) -> Result<Response<PurchaseOrderResponse>, Status> { Err(Status::unimplemented("")) }
    async fn get_purchase_order(&self, _r: Request<GetPurchaseOrderRequest>) -> Result<Response<PurchaseOrderDetail>, Status> { Err(Status::unimplemented("")) }
    async fn list_purchase_orders(&self, _r: Request<ListPurchaseOrdersRequest>) -> Result<Response<ListPurchaseOrdersResponse>, Status> { Err(Status::unimplemented("")) }
    async fn release_purchase_order(&self, _r: Request<ReleasePurchaseOrderRequest>) -> Result<Response<ReleasePurchaseOrderResponse>, Status> { Err(Status::unimplemented("")) }
