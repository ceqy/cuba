...
use std::sync::Arc;
use crate::application::commands::{ReceiveInvoiceCommand, MatchInvoiceCommand, PostInvoiceCommand, InvoiceItemCmd};
...
impl InvoiceProcessingService for IvServiceImpl {
    ...
    async fn receive_invoice(
        ...
        let gross = header.gross_amount.map(|a| Decimal::from_str(&a.value).unwrap_or_default()).unwrap_or_default();
        let tax = header.tax_amount.map(|a| Decimal::from_str(&a.value).unwrap_or_default()).unwrap_or_default();
        let cmd = ReceiveInvoiceCommand {
            ...
            gross_amount: gross,
            tax_amount: tax,
            items: req.items.into_iter().map(|i| InvoiceItemCmd {
                item_number: i.item_number,
                po_number: if i.purchase_order_number.is_empty() { None } else { Some(i.purchase_order_number) },
                po_item: if i.purchase_order_item == 0 { None } else { Some(i.purchase_order_item) },
                material: if i.material.is_empty() { None } else { Some(i.material) },
                quantity: i.quantity.map(|q| Decimal::from_str(&q.value).unwrap_or_default()).unwrap_or_default(),
                amount: i.amount.map(|a| Decimal::from_str(&a.value).unwrap_or_default()).unwrap_or_default(),
            }).collect(),
        };
        let id = self.handler.receive_invoice(cmd).await
    ...
    async fn match_invoice(
        ...
        let (success, _) = self.handler.match_invoice(cmd).await
            .map_err(|e| Status::internal(e.to_string()))?;
        Ok(Response::new(MatchInvoiceResponse {
            internal_invoice_id: req.internal_invoice_id,
            match_status: if success { MatchStatus::Success as i32 } else { MatchStatus::PriceVariance as i32 },
            variances: vec![],
        }))
    }
    ...
    async fn post_invoice(
        ...
        let invoice_id = uuid::Uuid::parse_str(&req.internal_invoice_id)
            .map_err(|_| Status::invalid_argument("Invalid UUID"))?;
        let cmd = PostInvoiceCommand { invoice_id, accept_variances: req.accept_variances };
        let doc_num = self.handler.post_invoice(cmd).await
            .map_err(|e| Status::internal(e.to_string()))?;
    ...
    async fn get_invoice(
        ...
        let invoice_id = uuid::Uuid::parse_str(&id_str)
            .map_err(|_| Status::invalid_argument("Invalid UUID"))?;
        let inv = self.repo.find_by_id(invoice_id).await
            .map_err(|e| Status::internal(e.to_string()))?
        ...
        Ok(Response::new(InvoiceDetail {
            ...
            header: Some(InvoiceHeader {
                ...
                gross_amount: Some(common_v1::MonetaryValue { value: inv.gross_amount.to_string(), currency_code: inv.currency.clone() }),
                tax_amount: None,
                currency: inv.currency,
                payment_terms: inv.payment_terms.unwrap_or_default(),
                header_text: inv.header_text.unwrap_or_default(),
            }),
            items: inv.items.into_iter().map(|i| InvoiceItem {
                item_number: i.item_number,
                ...
                quantity: Some(common_v1::QuantityValue { value: i.quantity.to_string(), unit_code: i.unit }),
                unit: "".to_string(),
                amount: Some(common_v1::MonetaryValue { value: i.amount.to_string(), currency_code: "CNY".to_string() }),
    ...
    // Stubs
    async fn park_invoice(&self, _r: Request<ParkInvoiceRequest>) -> Result<Response<ParkInvoiceResponse>, Status> { Err(Status::unimplemented("")) }
    async fn reverse_invoice(&self, _r: Request<ReverseInvoiceRequest>) -> Result<Response<ReverseInvoiceResponse>, Status> { Err(Status::unimplemented("")) }
    async fn get_processing_status(&self, _r: Request<GetProcessingStatusRequest>) -> Result<Response<common_v1::JobInfo>, Status> { Err(Status::unimplemented("")) }
}
