...
impl ProjectCostControllingService for PsServiceImpl {
    ...
    async fn post_direct_cost(
        ...
        let cmd = PostDirectCostCommand {
            ...
            description: if req.text.is_empty() {
                None
            } else {
                Some(req.text)
            },
    ...
    async fn get_project_cost_report(
        ...
        let req = request.into_inner();
        let (budget, actual) = self
            .repo
            .get_cost_report(&req.project_or_wbs_element)
            .await
            .map_err(|e| Status::internal(e.to_string()))?;
        ...
        Ok(Response::new(ProjectCostReport {
            ...
            planned_cost: None,
            actual_cost: Some(common_v1::MonetaryValue {
                value: actual.to_string(),
                currency_code: "CNY".to_string(),
            }),
            commitment_cost: None,
            budget: Some(common_v1::MonetaryValue {
                value: budget.to_string(),
                currency_code: "CNY".to_string(),
            }),
            available_amount: Some(common_v1::MonetaryValue {
    ...
    // Stubs
    async fn list_project_costs(
        &self,
        _r: Request<ListProjectCostsRequest>,
    ) -> Result<Response<ListProjectCostsResponse>, Status> {
        Err(Status::unimplemented(""))
    }
    async fn update_cost(
        &self,
        _r: Request<UpdateCostRequest>,
    ) -> Result<Response<PostDirectCostResponse>, Status> {
        Err(Status::unimplemented(""))
    }
}
