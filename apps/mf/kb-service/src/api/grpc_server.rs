...
impl KanbanService for KbServiceImpl {
    ...
    async fn create_kanban_control_cycle(
        ...
        let qty = cycle.quantity_per_kanban.map(|q| Decimal::from_str(&q.value).unwrap_or_default()).unwrap_or_else(|| Decimal::new(100, 0));
        let cmd = CreateCycleCommand {
            material: cycle.material,
            plant: cycle.plant,
            supply_area: if cycle.production_supply_area.is_empty() { None } else { Some(cycle.production_supply_area) },
            number_of_kanbans: cycle.number_of_kanbans,
            qty_per_kanban: qty,
        };
        let num = self.handler.create_cycle(cmd).await
    ...
    async fn get_kanban_control_cycle(
        ...
        let req = request.into_inner();
        let c = self.repo.find_cycle_by_number(&req.control_cycle_number).await
            .map_err(|e| Status::internal(e.to_string()))?
        ...
        Ok(Response::new(KanbanControlCycle {
            ...
            quantity_per_kanban: Some(common_v1::QuantityValue { value: c.qty_per_kanban.to_string(), unit_code: c.unit }),
    ...
    async fn change_kanban_status(
        ...
        let replenishment_doc = self.handler.change_status(cmd).await
            .map_err(|e| Status::internal(e.to_string()))?;
    ...
    async fn list_kanban_containers(
        ...
        let req = request.into_inner();
        let cycle = self.repo.find_cycle_by_number(&req.material).await
            .map_err(|e| Status::internal(e.to_string()))?;
        let containers = if let Some(c) = cycle {
            self.repo.list_containers_by_cycle(c.cycle_id).await
                .map_err(|e| Status::internal(e.to_string()))?
        } else {
            vec![]
        };
        Ok(Response::new(ListKanbanContainersResponse {
            containers: containers.into_iter().map(|k| KanbanContainer {
    ...
    async fn update_kanban_control_cycle(&self, _r: Request<UpdateControlCycleRequest>) -> Result<Response<ControlCycleResponse>, Status> { Err(Status::unimplemented("")) }
}
