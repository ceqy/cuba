...
impl ShopFloorExecutionService for SfServiceImpl {
    ...
    async fn cancel_confirmation(
        &self,
        _r: Request<CancelConfirmationRequest>,
    ) -> Result<Response<CancelConfirmationResponse>, Status> {
        Err(Status::unimplemented(""))
    }
    async fn post_material_consumption(
        &self,
        _r: Request<PostMaterialMovementRequest>,
    ) -> Result<Response<common_v1::StockMovementResponse>, Status> {
        Err(Status::unimplemented(""))
    }
    async fn post_goods_receipt_for_order(
        &self,
        _r: Request<PostMaterialMovementRequest>,
    ) -> Result<Response<common_v1::StockMovementResponse>, Status> {
        Err(Status::unimplemented(""))
