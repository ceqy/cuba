...
impl ProductionPlanningService for PpServiceImpl {
    ...
    // Stubs
    async fn simulate_mrp(
        &self,
        _r: Request<RunMrpRequest>,
    ) -> Result<Response<common_v1::JobInfo>, Status> {
        Err(Status::unimplemented(""))
    }
    async fn get_mrp_status(
        &self,
        _r: Request<GetMrpStatusRequest>,
    ) -> Result<Response<common_v1::JobInfo>, Status> {
        Err(Status::unimplemented(""))
    }
    async fn convert_planned_order(
        &self,
        _r: Request<ConvertPlannedOrderRequest>,
    ) -> Result<Response<ConvertPlannedOrderResponse>, Status> {
        Err(Status::unimplemented(""))
    }
    async fn get_work_center_capacity_load(
        &self,
        _r: Request<GetWorkCenterCapacityLoadRequest>,
    ) -> Result<Response<GetWorkCenterCapacityLoadResponse>, Status> {
        Err(Status::unimplemented(""))
    }
}
