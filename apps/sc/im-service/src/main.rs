...
use im_service::application::handlers::{GetStockOverviewHandler, PostStockMovementHandler};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // API
    let service = ImServiceImpl::new(post_handler, get_stock_handler);

    // Reflection Service
    let reflection_service = tonic_reflection::server::Builder::configure()
        .register_encoded_file_descriptor_set(
            im_service::api::proto::sc::im::v1::FILE_DESCRIPTOR_SET,
        )
        .build_v1()?;
