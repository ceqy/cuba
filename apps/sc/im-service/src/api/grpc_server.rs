...
use std::sync::Arc;
use crate::application::commands::{PostStockMovementCommand, StockMovementItemCommand, GetStockOverviewQuery};
use crate::application::handlers::{PostStockMovementHandler, GetStockOverviewHandler};
...
impl InventoryManagementService for ImServiceImpl {
    ...
    async fn post_stock_movement(
        ...
        let number = self.post_handler.handle(cmd).await
    ...
    async fn get_stock_overview(
        ...
        let req = request.into_inner();
        let stocks = self.get_stock_handler.handle(GetStockOverviewQuery {
            material: req.material,
            plant: req.plant,
            storage_location: if req.storage_locations.is_empty() { None } else { Some(req.storage_locations[0].clone()) },
        }).await.map_err(|e| Status::internal(e.to_string()))?;

        Ok(Response::new(GetStockOverviewResponse {
            items: stocks.into_iter().map(|s| StockOverviewItem {
                material: s.material,
                plant: s.plant,
                storage_location: s.storage_location,
                batch: s.batch,
                unrestricted_stock: Some(to_proto_qty(s.unrestricted_quantity, &s.base_unit)),
                quality_inspection_stock: Some(to_proto_qty(s.quality_inspection_quantity, &s.base_unit)),
    ...
    // Stubs
    async fn stream_stock_list(&self, _r: Request<ListStockRequest>) -> Result<Response<Self::StreamStockListStream>, Status> { Err(Status::unimplemented("")) }
    async fn check_stock_availability(&self, _r: Request<CheckStockAvailabilityRequest>) -> Result<Response<StockAvailabilityResponse>, Status> { Err(Status::unimplemented("")) }
    async fn reverse_material_document(&self, _r: Request<ReverseMaterialDocumentRequest>) -> Result<Response<common_v1::StockMovementResponse>, Status> { Err(Status::unimplemented("")) }
    async fn create_reservation(&self, _r: Request<CreateReservationRequest>) -> Result<Response<ReservationResponse>, Status> { Err(Status::unimplemented("")) }
    async fn cancel_reservation(&self, _r: Request<CancelReservationRequest>) -> Result<Response<ReservationResponse>, Status> { Err(Status::unimplemented("")) }
    async fn create_physical_inventory_document(&self, _r: Request<CreatePiDocumentRequest>) -> Result<Response<PiDocumentResponse>, Status> { Err(Status::unimplemented("")) }
    async fn enter_inventory_count(&self, _r: Request<EnterInventoryCountRequest>) -> Result<Response<EnterInventoryCountResponse>, Status> { Err(Status::unimplemented("")) }
    async fn post_inventory_difference(&self, _r: Request<PostInventoryDifferenceRequest>) -> Result<Response<common_v1::StockMovementResponse>, Status> { Err(Status::unimplemented("")) }
