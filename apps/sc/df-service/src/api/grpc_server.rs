...
impl DemandForecastingService for DfServiceImpl {
    ...
    async fn generate_forecast(
        ...
        let _plan_code = self.handler.generate_forecast(cmd).await
    ...
    async fn get_forecast(
        ...
        let req = request.into_inner();
        let plan = self.repo.find_by_code(&req.plan_id).await
            .map_err(|e| Status::internal(e.to_string()))?
        ...
        Ok(Response::new(ForecastPlan {
            ...
            periods: plan.periods.into_iter().map(|p| ForecastPeriod {
                start_date: Some(prost_types::Timestamp { seconds: p.start_date.and_hms_opt(0, 0, 0).unwrap().and_utc().timestamp(), nanos: 0 }),
                end_date: Some(prost_types::Timestamp { seconds: p.end_date.and_hms_opt(0, 0, 0).unwrap().and_utc().timestamp(), nanos: 0 }),
                forecasted_quantity: p.forecasted_quantity.map(|q| common_v1::QuantityValue { value: q.to_string(), unit_code: p.unit.clone() }),
                confidence_lower_bound: p.confidence_lower.map(|l| common_v1::QuantityValue { value: l.to_string(), unit_code: p.unit.clone() }),
                confidence_upper_bound: p.confidence_upper.map(|u| common_v1::QuantityValue { value: u.to_string(), unit_code: p.unit }),
            }).collect(),
            model_used: common_v1::ForecastModel::MovingAverage as i32,
            created_at: Some(prost_types::Timestamp { seconds: plan.created_at.timestamp(), nanos: 0 }),
        }))
    }
    ...
    async fn transfer_forecast_to_erp(
        ...
        let req = request.into_inner();
        let cmd = TransferCommand { plan_code: req.plan_id };
        let success = self.handler.transfer_forecast(cmd).await
            .map_err(|e| Status::internal(e.to_string()))?;
    ...
    async fn update_forecast_plan(&self, _r: Request<UpdateForecastPlanRequest>) -> Result<Response<ForecastPlanResponse>, Status> { Err(Status::unimplemented("")) }
    async fn compare_forecast_versions(&self, _r: Request<CompareForecastVersionsRequest>) -> Result<Response<ForecastComparisonResponse>, Status> { Err(Status::unimplemented("")) }
}
