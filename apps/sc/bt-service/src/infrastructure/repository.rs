...
impl BatchRepository {
    ...
    pub async fn find_by_material_batch(&self, mat: &str, batch: &str, plant: &str) -> Result<Option<Batch>> {
        let h = sqlx::query_as::<_, Batch>("SELECT * FROM batches WHERE material = $1 AND batch_number = $2 AND plant = $3")
            .bind(mat)
            .bind(batch)
            .bind(plant)
    ...
    pub async fn get_history(&self, batch_id: uuid::Uuid) -> Result<Vec<BatchHistoryEvent>> {
        let rows = sqlx::query_as::<_, BatchHistoryEvent>("SELECT * FROM batch_history WHERE batch_id = $1 ORDER BY event_time")
            .bind(batch_id)
